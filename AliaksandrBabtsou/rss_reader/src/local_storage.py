...
class LocalStorage():
    ...
    def read_url(self, url: str, date_filter: str, limit: int = None) -> Feed:
        ...
        def equal_date(item):
            ...
            if dt_str == date_filter:
                ...
                return False
        find_idx, find_in_storage = next(((idx, feed) for idx, feed in enumerate(feeds_storage.feeds) if feed.url == url), (None, None))
    ...
    def append(self, rss_data: Feed):
        ...
        with open(self._path, 'rb') as f:
            feeds_storage =  pickle.load(f)
        find_idx, find_in_storage = next(((idx, feed) for idx, feed in enumerate(feeds_storage.feeds) if feed.url == add_feed.url), (None, None))
        if find_in_storage is not None:
            find_in_storage.items.extend(add_feed.items)
            seen = set()            
            unique_items = [seen.add(item.item_title) or item for item in find_in_storage.items if item.item_title not in seen]
